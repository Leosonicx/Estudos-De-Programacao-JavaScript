...
for (let i = 0; i < randomValues.length; i++) {
  ...
  const isItemAString = typeof typeOfItem === 'string'
  const isItemABoolean = typeof typeOfItem === 'boolean'
  ...
  if (isItemAString) {
    stringsAmount++
    firstFourStrings.push(typeOfItem) 
  }
...
switch (drinkType) {
  case 'água':
    drinkMessage = `Substânci aquímica cujas moléculas são formadas por dois átomos de hidrogênio e um de oxigênio.`
    break
  case 'refrigerante':
